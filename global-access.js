...
exports.deployGlobalAccessMesh = deployGlobalAccessMesh;
const angels_1 = require("cloudflare/angels");
const heaven_auth_1 = require("firebase/heaven-auth");
const client_core_1 = require("@antigravity/client-core");
async function deployGlobalAccessMesh() {
    ...
    await angels_1.CloudflareZeroTrust.createAccessPolicy({
    ...
    heaven_auth_1.FirebaseAuth.setCustomUserClaims({
    ...
    const handleClientRequest = async (userAuth, encryptedFileId) => {
        // A. Verify clearance level
        if (!heaven_auth_1.FirebaseAuth.verifyClearance(userAuth, encryptedFileId)) {
        ...
        const sessionKey = await client_core_1.ClientWasmEnclave.generateEphemeralKey(userAuth.id);
        return {
            status: "AUTHORIZED",
